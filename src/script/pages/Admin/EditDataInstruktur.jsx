...
import { PutDataUserAdmin, GetInstrukturById} from "../../config/ApiService";
import { useEffect, useState } from "react";
import Swal from "sweetalert2";
import { useParams, useNavigate } from "react-router-dom";

const EditDataInstruktur = () => {
  const navigate = useNavigate();
  const { idInstruktur} = useParams();
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const getData = await GetInstrukturById(idInstruktur);
      setForm({
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const PostData = await PutDataUserAdmin(idInstruktur, form);
      await Swal.fire({
        title: "Berhasil",
        text: "Berhasil mengubah data instruktur",
        icon: "success",
      }).then(() => {
...
export default EditDataInstruktur;
