...
import { GetPelatihan, GetPenilaianById, PutPenilaian } from "../../config/ApiService";
import { useEffect, useState } from "react";
import Swal from "sweetalert2";
import { useNavigate, useParams } from "react-router-dom";
import Select from "react-select";

const EditDataPenilaian = () => {
  ...
  const [valuePelatihan, setValuePelatihan] = useState([]);
  ...
  const getData = async () => {
    ...
    try {
      ...
      setFormPelatihan(getDataPenilaian)
      setPelatihan(data);
      setValuePelatihan({
        value: getDataPenilaian.pelatihan.id,
        label: getDataPenilaian.pelatihan.nama,
      })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const PutData = await PutPenilaian(idPenilaian, data);
      await Swal.fire({
        title: "Berhasil",
        text: "Berhasil mengubah data penilaian",
        icon: "success",
      }).then(() => {
...
export default EditDataPenilaian;
