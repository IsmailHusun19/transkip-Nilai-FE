...
const EditDataPeriode = () => {
  const { idPeriode } = useParams();
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const getDataPeriode = await GetPeriodeById(idPeriode);
      console.log(getDataPeriode.data);
      setForm({
        nama: getDataPeriode.data.nama,
        aktif: getDataPeriode.data.aktif,
      });
    } catch (e) {
      console.error(e);
    } finally {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const PutDataPeriode = await PutPeriode(idPeriode, form);
      await Swal.fire({
        title: "Berhasil",
        text: "Berhasil edit periode",
        icon: "success",
      }).then(() => {
        setForm({
          nama: "",
...
export default EditDataPeriode;
