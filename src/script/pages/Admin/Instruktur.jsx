...
import { GetInstruktur, DeleteInstruktur } from "../../config/ApiService";
...
const Instruktur = () => {
  ...
  useEffect(() => {
    if (!loading && instruktur) {
      const filteredInstruktur = instruktur.filter((item) =>
        item.nama.toLowerCase().includes(searchTerm.toLowerCase())
      );
      if (searchTerm) {
        setDataSearch(filteredInstruktur);
      } else {
        setDataSearch(instruktur);
      }
    }
  }, [searchTerm, instruktur, loading]);
            ...
            >
              <div className="relative flex justify-end mb-4">
                ...
                <div
                  ...
                  onClick={() => navigate("/admin/instruktur/tambah")}
                >
                            ...
                            <>
                              <tr>
                                <td
                                  ...
                                  className="text-center font-medium text-gray-600 align-middle h-32"
                                >
                                  Data instruktur tidak ada
                                </td>
                              </tr>
                            </>
                          ) : (
                            <>
                              {dataSearch.map((instruktur) => (
                                <tr
                                  key={instruktur.id}
                                ...
                                >
                                  ...
                                  <td className="px-6 py-4 flex gap-2">
                                    <FaPencilAlt
                                      onClick={() =>
                                        navigate(
                                          `/admin/instruktur/edit/${instruktur.id}`
                                        )
                                      }
                                      className="border border-black w-5 h-5 p-1 box-content cursor-pointer hover:text-gray-800 rounded-md"
                                    />{" "}
                                    <FaTrash onClick={() => deleteInstruktur(instruktur.id)} className="border border-black w-5 h-5 p-1 box-content cursor-pointer hover:text-gray-800 rounded-md" />
                                  </td>
                                </tr>
                              ))}
                            </>
