...
import { GetPelatihanByInstruktur } from "../../config/ApiService";
import { useEffect, useState } from "react";
import { FaEye } from "react-icons/fa";

const StudiPembelajaranInstruktur = () => {
  ...
  const getData = async () => {
    setLoading(true);
    try {
      const getDataPelatihan = await GetPelatihanByInstruktur();
                          ...
                          >
                            ...
                            <td className="px-6 py-4">
                              {(() => {
                                ...
                                const periode =
                                  pelatihan.periode.nama.toLowerCase();
                                  console.log(periode)
                                let tahun = 0;
                                let bulan = 0;
                                const periodeRegex =
                            ...
                            <td className="px-6 py-4 flex gap-2">
                              <FaEye onClick={() => navigate(`/detail-studi-pembelajaran-instruktur/${pelatihan.id}`)} className="border border-black w-5 h-5 p-1 box-content cursor-pointer hover:text-gray-800 rounded-md" />{" "}
                            </td>
                          </tr>
                        ))}
                      </>
...
export default StudiPembelajaranInstruktur;
